...
update_agent_grid <- function(grid, interest_rate) {
  ...
  p1_raw <- 0.33 - 0.2 * interest_rate
  p2_raw <- 0.34 - 0.1 * interest_rate
  p3_raw <- 0.33 + 0.3 * interest_rate
  p1_raw <- max(0, p1_raw)
  p2_raw <- max(0, p2_raw)
  p3_raw <- max(0, p3_raw)
...
update_arms_market <- function(M, S, B, sentiment_index, interest_rate, dt) {
  k_buy <- k0 * (1.0 + alpha * sentiment_index)
  k_sell <- k0 * (1.0 - alpha * sentiment_index)
